...
object Exportable {
  private def exportJsObject[T](value: T)(implicit exportable: Exportable[T]): JsValue = exportable.exportJsObject(value)
  ...
  implicit val variableIntExportable: Exportable[Variable[Int]] = value => Json.obj(
    "cls" -> value.cls,
    "name" -> value.name,
    "value" -> value.value
  )

  implicit val variableDoubleExportable: Exportable[Variable[Double]] = value => Json.obj(
    "cls" -> value.cls,
    "name" -> value.name,
    "value" -> value.value
  )
  ...
  implicit val progressBarExportable: Exportable[ProgressBar] = value => Json.obj(
    "cls" -> value.cls,
    "name" -> value.name,
    "value" -> value.value,
    "total" -> value.total
  ...
  implicit val loggerExportable: Exportable[Logger] = value => Json.obj(
    "cls" -> value.cls,
    "name" -> value.name,
    "value" -> value.value
  )

  implicit val lineChartExportable: Exportable[LineChart] = value => Json.obj(
    "cls" -> value.cls,
    "name" -> value.name,
    "value" -> value.value.map(datum => Json.obj(
      "x" -> datum.x,
      "y" -> datum.y,
      "label" -> datum.label
    )),
